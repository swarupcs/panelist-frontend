...
import {
  ...
  ChevronRight, UserCheck, Download, Eye, RefreshCw, Plus,
} from 'lucide-react'
...
import { formatRelative, formatDate, getInitials } from '@/utils/formatters'
...
export function AdminUsersTab() {
  ...
  const handleSearch = useCallback((val: string) => {
    ...
    ;(window as any).__adminSearch = setTimeout(() => {
      setDebSearch(val)
      setPage(1)
    }, 350)
  }, [])
    ...
    <div className="space-y-4 animate-fade-in">
      ...
      <Card>
        <CardContent className="pt-4 pb-3">
          <div className="flex flex-wrap gap-2 items-center">
            ...
            <select
              value={status}
              onChange={(e) => { setStatus(e.target.value as UserFilterStatus); setPage(1) }}
              className="h-8 rounded-lg border border-border bg-input px-2 text-sm text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            >
            ...
            <Button
              variant={suspicious ? 'default' : 'outline'}
              size="sm"
      ...
      {selected.size > 0 && (
        <div className="flex items-center gap-3 rounded-lg border border-border bg-card px-4 py-2.5">
          ...
          <Button
            variant="destructive"
            size="sm"
            onClick={() => openDialog('bulk-ban', '__bulk__')}
          >
      ...
      <Card>
        ...
        <CardContent className="p-0">
          {isLoading ? (
            <div className="py-10"><LoadingScreen /></div>
          ) : isError ? (
            ...
            <>
              ...
              {data?.users.map((user) => (
                <UserRow
                  ...
                  onMarkSuspicious={() =>
                    markSuspicious.mutate({ userId: user.id, isSuspicious: !user.isSuspicious })
                  }
...
function UserRow({
  user, selected, onSelect, onViewDetail,
  onBan, onUnban, onSuspend, onUnsuspend,
  onDelete, onResetPassword, onEditNotes,
  onMarkSuspicious, onChangeRole, onExport,
}: {
    ...
    >
      ...
      <button className="flex-1 min-w-0 text-left" onClick={onViewDetail}>
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {user.isSuspicious && (
            <Badge className="text-xs py-0 bg-orange-500/10 text-orange-400 border-orange-500/20">
              Suspicious
            </Badge>
          )}
          {!user.emailVerified && (
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-44">
          ...
          {user.isBanned ? (
            <DropdownMenuItem onClick={onUnban} className="text-green-400">
          ...
          {user.isSuspended ? (
            <DropdownMenuItem onClick={onUnsuspend}>
              <ShieldOff className="size-3.5" /> Unsuspend
            </DropdownMenuItem>
          ) : (
          ...
          <DropdownMenuItem onClick={onMarkSuspicious}>
            ...
            {user.isSuspicious ? 'Unmark' : 'Mark'} Suspicious
          </DropdownMenuItem>
...
function UserDetailPanel({
  userId, onClose, onBan, onUnban, onDelete,
}: {
  userId: string
  onClose: () => void
  onBan: () => void
                ...
                <div className="space-y-1 max-h-40 overflow-y-auto pr-1">
                  {data.recentActivity.slice(0, 10).map((act) => (
                    <div key={act.id} className="flex items-center justify-between gap-2">
                      ...
                      <Badge
                        variant={act.statusCode < 400 ? 'secondary' : 'destructive'}
...
function ActionDialog({
  ...
  > = {
    ...
    delete:        { title: 'Delete Account?',    description: 'Permanently deletes all data. Irreversible.',  confirmLabel: 'Delete',    destructive: true  },
  ...
  const isValid       = needsReason
    ? dialog.reason.trim().length > 0
    : needsPassword
      ? dialog.newPassword.length >= 8
      : true
